...
class AttendanceHistoryController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            \Log::error('AttendanceHistory index error: ' . $e->getMessage());
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Gagal memuat data riwayat presensi.'
                ], 500);
            }
            return back()->with('error', 'Gagal memuat data riwayat presensi. Silakan coba lagi.');
        }
    }
