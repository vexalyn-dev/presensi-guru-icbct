            ...
            <div class="card p-6">
                ...
                <form action="{{ route('settings.attendance') }}" method="POST" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <!-- Grace Period Antar Kelas -->
                        @php $currentGrace = old('class_switch_grace_period', $settings['attendance']['class_switch_grace_period'] ?? 5); @endphp
                        <div x-data="{
                            openGrace: false,
                            selectedGracePeriod: '{{ $currentGrace }}',
                            gracePeriodOptions: [
                                { value: '0',  name: 'Tidak ada (langsung bisa scan)' },
                                { value: '2',  name: '2 menit' },
                                { value: '3',  name: '3 menit' },
                                { value: '5',  name: '5 menit (recommended)' },
                                { value: '10', name: '10 menit' },
                                { value: '15', name: '15 menit' },
                                { value: '30', name: '30 menit' },
                            ]
                        }" @click.outside="openGrace = false">
                            <label class="block text-sm font-semibold text-navy-800 dark:text-white mb-2">
                                Grace Period Antar Kelas (Menit)
                            </label>
                            <div class="relative">
                                <button type="button" @click="openGrace = !openGrace"
                                        class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-navy-800 dark:focus:ring-gold-500 flex items-center justify-between hover:border-navy-300 dark:hover:border-gold-600 transition-all">
                                    <div class="flex items-center gap-3">
                                        <div class="w-6 h-6 rounded-lg flex items-center justify-center bg-orange-100 dark:bg-orange-900/30">
                                            <i data-lucide="timer" class="w-4 h-4 text-orange-600 dark:text-orange-400"></i>
                                        </div>
                                        <span class="text-slate-700 dark:text-slate-300 font-medium"
                                              x-text="(gracePeriodOptions.find(o => o.value == selectedGracePeriod)?.name) || selectedGracePeriod + ' menit'"></span>
                                    </div>
                                    <svg class="w-4 h-4 text-slate-400 transition-transform duration-200" :class="{'rotate-180': openGrace}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                    </svg>
                                </button>

                                <div x-show="openGrace"
                                     ...
                                     x-cloak>
                                    <div class="overflow-y-auto" style="max-height: 240px;">
                                        <template x-for="option in gracePeriodOptions" :key="option.value">
                                            <button type="button"
                                                    @click="selectedGracePeriod = option.value; openGrace = false"
                                                    class="w-full px-4 py-3 text-left hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors flex items-center gap-3 border-b border-slate-100 dark:border-slate-700 last:border-0"
                                                    :class="selectedGracePeriod == option.value ? 'bg-navy-50 dark:bg-navy-900/30' : ''">
                                                <div class="w-6 h-6 rounded-lg flex items-center justify-center bg-orange-100 dark:bg-orange-900/30">
                                                    <i data-lucide="timer" class="w-4 h-4 text-orange-600 dark:text-orange-400"></i>
                                                </div>
                                                <span class="text-sm font-medium text-slate-700 dark:text-slate-300 flex-1" x-text="option.name"></span>
                                                <svg x-show="selectedGracePeriod == option.value" class="w-4 h-4 text-navy-800 dark:text-gold-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                                                </svg>
                                            </button>
                                        </template>
                                ...
                                <input type="hidden" name="class_switch_grace_period" :value="selectedGracePeriod">
                                <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">Waktu minimal antara scan OUT dan IN kelas berikutnya</p>
                            </div>
                        </div>
        ...
        <form action="{{ route('settings.apk') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card overflow-hidden">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="flex items-center gap-4 p-4 rounded-2xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/50">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[10px] text-green-500 dark:text-green-500 mt-0.5">Diupload {{ $apkSetting->apk_uploaded_at->diffForHumans() }}</p>
                            @endif
                        </div>
                        <form action="{{ route('settings.apk.delete') }}" method="POST" onsubmit="return confirm('Hapus APK ini?')">
                            @csrf @method('DELETE')
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-navy-800 dark:text-white mb-2">
                            Upload File APK
                            <span class="text-[10px] font-normal text-slate-400 ml-1">(max 100MB · .apk) — metadata otomatis terisi</span>
                        </label>
                        <div class="relative border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-2xl p-6 text-center hover:border-gold-400 transition-colors cursor-pointer"
                            ...
                            <input type="file" name="apk_file" accept=".apk" id="apk_file_input" x-ref="apkInput"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-navy-800 dark:text-white mb-1.5">
                                Nama Aplikasi
                                <span class="text-[10px] font-normal text-slate-400 ml-1">auto dari APK</span>
                            </label>
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"><i data-lucide="type" class="w-4 h-4"></i></span>
                                <input type="text" name="apk_name" value="{{ old('apk_name', $apkSetting->apk_name ?? 'ICB CT Presensi') }}"
                                       placeholder="ICB CT Presensi"
                                       class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-navy-800 dark:text-white focus:ring-2 focus:ring-gold-400 focus:border-transparent outline-none transition">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-navy-800 dark:text-white mb-1.5">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="apk_version" value="{{ old('apk_version', $apkSetting->apk_version ?? '1.0.0') }}"
                                       placeholder="1.0.0"
                                       class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-navy-800 dark:text-white focus:ring-2 focus:ring-gold-400 focus:border-transparent outline-none transition">
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="apk_min_android" value="{{ old('apk_min_android', $apkSetting->apk_min_android ?? 'Android 8.0+') }}"
                                       placeholder="Android 8.0+"
                                       class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-navy-800 dark:text-white focus:ring-2 focus:ring-gold-400 focus:border-transparent outline-none transition">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-navy-800 dark:text-white mb-1.5">Changelog / Catatan Versi</label>
                            <div class="relative">
                                ...
                                <textarea name="apk_changelog" rows="2" placeholder="Perubahan di versi ini..."
                                          class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-navy-800 dark:text-white focus:ring-2 focus:ring-gold-400 focus:border-transparent outline-none transition resize-none">{{ old('apk_changelog', $apkSetting->apk_changelog) }}</textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="px-6 py-4 bg-slate-50 dark:bg-slate-700/50 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                    <button type="submit"
                            class="flex items-center gap-2 px-6 py-2.5 bg-navy-800 hover:bg-navy-700 dark:bg-gold-400 dark:hover:bg-gold-300 text-white dark:text-navy-900 rounded-xl font-bold text-sm transition-all shadow-lg hover:-translate-y-0.5">
                        <i data-lucide="save" class="w-4 h-4"></i>
                        Simpan Pengaturan APK
                    </button>
                </div>
            </div>
        </form>
