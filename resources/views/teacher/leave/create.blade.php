    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 sm:mb-8">
        <div class="flex items-center gap-3">
            <a href="{{ route('teacher.leave') }}"
                class="group flex items-center gap-2 px-3 py-2.5 sm:px-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-300 text-sm font-medium transition-all duration-200 shadow-sm hover:shadow-md flex-shrink-0">
                <i data-lucide="arrow-left" class="w-4 h-4 transition-transform group-hover:-translate-x-1"></i>
                <span class="hidden sm:inline">Kembali</span>
            </a>
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-navy-800 dark:text-white">Ajukan Izin / Sakit</h1>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Isi formulir untuk mengajukan izin</p>
            </div>
        </div>
    </div>

    <!-- Row 1: Panduan & Timeline — hidden di mobile, tampil di lg -->
    <div class="hidden lg:grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="card p-6 animate-slide-up" style="animation-delay: 0.1s">
            ...
            <div class="space-y-4">
                <div class="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors group">
                    <div class="w-6 h-6 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform">
                ...
                <div class="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors group">
                    ...
                    <p class="text-xs text-slate-600 dark:text-slate-400">Lampirkan surat dokter untuk pengajuan sakit</p>
                </div>
                <div class="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors group">
                    <div class="w-6 h-6 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform">
                        <i data-lucide="check" class="w-3.5 h-3.5 text-green-600 dark:text-green-400"></i>
                    ...
                    <div>
                        ...
                        <div class="relative" @click.outside="open = false">
                            <button type="button" 
                                ...
                                <div class="flex items-center gap-3">
                                    <i :data-lucide="getIcon(selectedType)" 
                                       class="w-4 h-4"
                                       :class="selectedType ? 'text-navy-600 dark:text-gold-400' : 'text-slate-400'"></i>
                                    <span class="text-slate-700 dark:text-slate-300" 
                                          x-text="getLabel(selectedType) || 'Pilih jenis pengajuan...'"></span>
                                </div>
                            ...
                            <div x-show="open" 
                                ...
                                <div class="max-h-60 overflow-y-auto p-1.5 space-y-1">
                                    <button type="button"
                                            ...
                                            :class="selectedType === 'izin' ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'">
                                        <i data-lucide="file-check" class="w-4 h-4" :class="selectedType === 'izin' ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500'"></i>
                                        <div class="flex-1">
                                            <p class="font-medium">Izin</p>
                                            <p class="text-[10px] opacity-70">Keperluan pribadi</p>
                                        </div>
                                        <i data-lucide="check" class="w-4 h-4" x-show="selectedType === 'izin'"></i>
                                    </button>

                                    <button type="button"
                                            @click="selectType('sakit'); open = false"
                                            class="w-full px-3 py-2.5 rounded-lg text-left text-sm transition-all flex items-center gap-3 group"
                                            :class="selectedType === 'sakit' ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 'hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'">
                                        <i data-lucide="pill" class="w-4 h-4" :class="selectedType === 'sakit' ? 'text-green-600' : 'text-slate-400 group-hover:text-green-500'"></i>
                                        <div class="flex-1">
                                            <p class="font-medium">Sakit</p>
                                            <p class="text-[10px] opacity-70">Dengan surat dokter</p>
                                        </div>
                                        <i data-lucide="check" class="w-4 h-4" x-show="selectedType === 'sakit'"></i>
                                    </button>
                                </div>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <label for="attachment" 
                                   ...
                                   :class="!fileName ? '' :
                                            fileType === 'pdf' ? 'border-red-400 bg-red-50 dark:bg-red-900/10' :
                                            fileType === 'word' ? 'border-blue-400 bg-blue-50 dark:bg-blue-900/10' :
                                            fileType === 'powerpoint' ? 'border-orange-400 bg-orange-50 dark:bg-orange-900/10' :
                                            fileType === 'image' ? 'border-green-400 bg-green-50 dark:bg-green-900/10' :
                                            'border-slate-400 bg-slate-50 dark:bg-slate-900/10'">
                                ...
                                <template x-if="!fileName">
                                    <div class="flex flex-col items-center text-center">
                                        ...
                                        <p class="text-[10px] text-slate-500 dark:text-slate-400">PDF, JPG, PNG, DOC, PPT • Max 2MB</p>
                                    </div>
                                </template>
                                ...
                                <template x-if="fileName">
                                    <div class="flex items-start gap-4 w-full">
                                        ...
                                        <template x-if="fileType === 'image'">
                                            <div class="relative w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 border-2 border-green-400">
                                                <img :src="filePreview" 
                                        ...
                                        <!-- Icon untuk dokumen PDF -->
                                        <template x-if="fileType === 'pdf'">
                                            <div class="w-16 h-16 bg-red-100 dark:bg-red-900/30 rounded-lg flex flex-col items-center justify-center flex-shrink-0 border-2 border-red-300 dark:border-red-700">
                                        ...
                                        <!-- Icon untuk dokumen Word -->
                                        <template x-if="fileType === 'word'">
                                            <div class="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex flex-col items-center justify-center flex-shrink-0 border-2 border-blue-300 dark:border-blue-700">
                                                <svg class="w-8 h-8 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                                                </svg>
                                                <span class="text-[8px] font-bold text-blue-600 dark:text-blue-400 mt-0.5">WORD</span>
                                            </div>
                                        </template>

                                        <!-- Icon untuk dokumen PowerPoint -->
                                        <template x-if="fileType === 'powerpoint'">
                                            <div class="w-16 h-16 bg-orange-100 dark:bg-orange-900/30 rounded-lg flex flex-col items-center justify-center flex-shrink-0 border-2 border-orange-300 dark:border-orange-700">
                                                <svg class="w-8 h-8 text-orange-600 dark:text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                                                </svg>
                                                <span class="text-[8px] font-bold text-orange-600 dark:text-orange-400 mt-0.5">PPT</span>
                                            </div>
                                        </template>

                                        <!-- Icon untuk dokumen lainnya -->
                                        <template x-if="fileType === 'other'">
                                            <div class="w-16 h-16 bg-slate-100 dark:bg-slate-700 rounded-lg flex flex-col items-center justify-center flex-shrink-0 border-2 border-slate-300 dark:border-slate-600">
                                                <svg class="w-8 h-8 text-slate-600 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                                                </svg>
                                                <span class="text-[8px] font-bold text-slate-600 dark:text-slate-400 mt-0.5">FILE</span>
                                            </div>
                                        </template>
                                        ...
                                        <div class="flex-1 min-w-0 pt-1">
                                            ...
                                            <p class="text-[10px]" 
                                               :class="fileType === 'pdf' ? 'text-red-600 dark:text-red-400' :
                                                        fileType === 'word' ? 'text-blue-600 dark:text-blue-400' :
                                                        fileType === 'powerpoint' ? 'text-orange-600 dark:text-orange-400' :
                                                        fileType === 'image' ? 'text-green-600 dark:text-green-400' :
                                                        'text-slate-600 dark:text-slate-400'">
                                                File terpilih • <span x-text="fileSize"></span>
                                            </p>
                                        </div>

                                        <button type="button" 
                                                @click.stop="clearFile"
                                                class="p-1.5 text-slate-400 hover:text-red-500 transition-colors flex-shrink-0 mt-1">
            ...
            <div class="mb-4">
                <label class="flex items-start gap-3 p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors shadow-sm">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400">Dengan mengajukan ini, saya menyetujui kebijakan sekolah terkait izin dan sakit</p>
                    </div>
                </label>
            </div>
